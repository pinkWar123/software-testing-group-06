...
description: Guided AI-assisted JMeter performance-test workflow (Load/Stress/Spike/Soak) with mandatory human review, smoke-testing, and two-pass .jtl log analysis. Use when the user asks to design/build/run a performance test plan, analyze JMeter results, or set up a continuous performance-testing pipeline.
---
...
Testing assignment on the EShop SUT (see
`HW05_PerformanceTesting/agent-skills/performance-testing-skill/SKILL.md` in that
project for the full worked example and rationale); reusable on a different
endpoint group or a different SUT with the same shape of problem.
...
- **Never invent a fake defect during review, but do document real ones.** If
  review finds nothing wrong, say so honestly rather than manufacture a defect —
  but do keep testing at successively higher scale (smoke → full run), since
  some bugs (like the assertion issue above) only show up once you revisit an
  already-transitioned record, which a 1-iteration smoke test won't naturally
  hit.
- **No fabricated evidence, ever.** `.jtl` logs, HTML reports, resource-monitor
  ...
  the human operator. You may run non-evidentiary smoke tests to validate a
  plan, but never the official recorded run, and never invent numbers.
...
8. **Hand off a live-execution runbook**, not silent execution. Never press
   "go" on the officially-graded run yourself — write exact commands, seeding
   steps, and screenshot/recording checkpoints for the human to execute and
   capture.
9. **Analyze the resulting `.jtl` logs in two passes.** First pass: a genuine,
   undirected read of the metrics (thresholds, p95/p99, error rate,
...
10. **Judge every proposed optimization** (index, connection pool, caching,
...
- You cannot capture screenshots, record video, or press "start" on a graded
  run — the human operator does all live execution and evidence capture. Batch
  and clearly checklist these requests in a single runbook document rather than
  asking for one at a time mid-flow.
...
- Commit after each concrete, describable unit of work (one scenario's draft,
  one scenario's fix, one analysis pass) — never batch unrelated steps into one
  commit.

## Example invocation

"Run the performance-testing skill for the FR-07 shopping-cart workflow, JMeter,
against this repo's backend." You would: read the cart/checkout handlers for any
non-obvious behavior first, propose one shared login→cart→checkout workflow,
design and get sign-off on Load parameters, build and smoke-test the `.jmx`,
review it against real JMeter success semantics, repeat for Stress/Spike, hand
off a runbook, then (once real `.jtl`s exist) run the two-pass log analysis and
